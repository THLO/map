...
* `#` is used as the placeholder for the current file including the full path but without the file extension.
* `&` is used as the placeholder for the full path to the file (excluding the file name).
* `%` is used as the placeholder for the extension of the file including `.`.
* `$` is used to refer to an internal counter, incremented after each command.

For example, if the current file is `path/to/file/filename.ext` and this is the `8th` file that is being processed, then
* `_` is replaced by `path/to/file/filename.ext`.
* `#` is replaced by `path/to/file/filename`.
